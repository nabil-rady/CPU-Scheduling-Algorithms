...
def show_widgets(event):
    ...
    def process_details_widgets():
        ...
        brust_time_label = tk.Label(process_details_frame, text = 'Enter process brust time:').grid(row=3, column=0, padx=5)
        global brust_time_entry
        brust_time_entry = tk.Entry(process_details_frame, width=15, borderwidth=5)
        ...
        if selected_algorithm.get() == scheduling_algorithms[4] or selected_algorithm.get() == scheduling_algorithms[5]:
            brust_time_label = tk.Label(process_details_frame, text = 'Enter process priority:').grid(row=4, column=0, padx=5)
